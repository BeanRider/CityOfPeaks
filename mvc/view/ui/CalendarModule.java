...
public class CalendarModule implements Interactable {
  ...
  private Dimension parentContainer = new Dimension(0, 0);

  /** x = row; y = col */
  private Optional<PVector> selectedCell = Optional.empty();
  private AbstractButton leftButton;
  private AbstractButton rightButton;

  /**
   * The calendar page to draw
   */
  ...
  public CalendarModule(int xLoc, int yLoc, Dimension parentSize,
                        PImage leftArrow, PImage rightArrow, DateTime currentTime) {
    cornerXY = new Point(xLoc, yLoc);
    parentContainer = new Dimension(parentSize);
  ...
  public Optional<DateTime> computeHoveredDate() {
  ...
  @Override
  public void render(CityOfPeaks_View parent, CityOfPeaks_DataModel dataModel) {
    ...
    if (isVisible()) {
      ...
      DateTime currentTime = new DateTime(
              dataModel.getTimeline().getCurUnix() * 1000L,
              DateTimeZone.forID("America/New_York"));

      // First Row: Label "Month Year" with left and right controls:
      parent.fill(255);
      parent.textAlign(CENTER, CENTER);
      parent.textFont(CityOfPeaks_View.FCODA_12, 11);
      ...
      leftButton.render(parent, dataModel);
      rightButton.render(parent, dataModel);
...
//      System.out.println(gridEnd);
      ...
      parent.textFont(CityOfPeaks_View.FHelB_12, 12);
  ...
  private void renderCell(CityOfPeaks_View parent, DateTime currentTime, DateTime gridDay, int row, int column) {
    ...
    if (currentTime.getDayOfMonth() == gridDay.getDayOfMonth()
      ...
      // THIS MONTH of THIS YEAR, BUT NOT TODAY
      parent.fill(255);
      parent.text(gridDay.getDayOfMonth(),
              cornerXY.x + column * colWidth + colWidth / 2,
              cornerXY.y + row * rowHeight + rowHeight / 2 + 1);
  ...
  public void mousePressedOutsideAction(CityOfPeaks_Controller controller, MouseEvent e) {
    ...
    actionSuite.actPressedOutside(controller, e);
  }
